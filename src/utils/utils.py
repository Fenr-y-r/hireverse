...
def denormalize_landmarks_without_Z(landmark: NormalizedLandmark, img) -> Tuple[int, int]:
    """
    Denormalize the landmarks to the original image size and ignores Z axis.
    """
    img_w , img_h = img.shape[1], img.shape[0]
    ordered_pair= landmark
    return (denormalize_int(ordered_pair.x, img_w), denormalize_int(ordered_pair.y, img_h))
